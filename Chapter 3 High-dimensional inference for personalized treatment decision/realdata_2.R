...
for(cv_rep_index in 1:cv_reps_num){
  ...
  rand_draw_index_train=sample(1:sample_size_org,280,replace=F)
  ...
  train_X=refit_newX[rand_draw_index_train,]
  test_y=refit_y[rand_draw_index_test]
  test_X=refit_newX[rand_draw_index_test,]
  

  
  refit_model=pvalue_de(train_X,train_y)
  
  selected_model=refit_model$sequence[(refit_model$pvalue<=0.05)]

  
  if(length(unique(c(p+1,p+selected_model)))==1){
    
    ls_X_train=train_X[,p+1]
    
    LS_coef=lm(train_y~ls_X_train -1)$coefficients
    LS_coef=as.numeric(LS_coef)
    
    temp= rep(LS_coef,length(rand_draw_index_test))
    
    decision_SER_test=(temp>0)
    decision_BUP_test=(temp<0)
  ...
  if(length(unique(c(p+1,p+selected_model)))>1){
    ...
    ls_X_train=train_X[,c(unique(c(selected_model,p+1,p+selected_model)))]
    
    LS_coef=lm(train_y~ls_X_train -1)$coefficients
    LS_coef=as.numeric(LS_coef)
    
    temp= refit_X_intercept[rand_draw_index_test,c(unique(c(1,selected_model)))] %*%
      LS_coef[(length(selected_model)+1):(length(selected_model)*2+1)]
