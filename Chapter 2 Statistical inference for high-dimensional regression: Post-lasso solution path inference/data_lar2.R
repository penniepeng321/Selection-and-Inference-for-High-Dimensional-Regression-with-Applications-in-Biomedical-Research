...
true_included_JPT_CHB=c("rs16981663_T","rs9981984_G","rs2832159_A","rs1999321_C","rs2832224_G",
                        "rs2245431_T","rs7282280_T")
filter_index=match(true_included_JPT_CHB[1],colnames(JPT_CHB_data))
for( hjk in 2:length(true_included_JPT_CHB)){
  filter_index=append(filter_index,match(true_included_JPT_CHB[hjk],colnames(JPT_CHB_data)))
}

count=rep(0,length(true_included_JPT_CHB))

for( hjk in 1:length(true_included_JPT_CHB)){
  if(is.na(sum(JPT_CHB_data[,filter_index[hjk]])) == T){
    tempp=mean(JPT_CHB_data[,filter_index[hjk]],na.rm = T)
    for(lkj in 1:90){
      if(is.na(JPT_CHB_data[lkj,filter_index[hjk]])==T){
        count[hjk]=count[hjk]+1
        JPT_CHB_data[lkj,filter_index[hjk]]=rbinom(1,1,tempp)
      }
    }
  }
}
...
set.seed(1000)
temp=sample(setdiff(colnames(JPT_CHB_data[, is.na(colSums(JPT_CHB_raw[, -(1 : 6)])) == F]),
                    true_included_JPT_CHB),
            6000-length(true_included_JPT_CHB))
for(hjk in 1:length(temp)){
  filter_index=append(filter_index,match(temp[hjk],colnames(JPT_CHB_data)))
}
length(filter_index)
...
#             100-length(true_included_JPT_CHB))
# for(hjk in 1:length(temp)){
#   filter_index=append(filter_index,match(temp[hjk],colnames(JPT_CHB_filter)))
# }
# length(filter_index)

JPT_CHB_filter <- JPT_CHB_data[filter_index]
...
#   res1=covTest(l1,X,y,sigma.est=1)$results
#   act1=res1[,1]
#   TS1=res1[,2]
#   #drop NAs
#   chos=!is.na(TS1)
...
for(hj in 1:sim){
  ...
  #Um=sort(runif(m-1,0,1),decreasing = F)
  Um=qts
  Vm[hj]=max((((1:(m-1))/(m-1)-Um)/sqrt(Um*(1-Um)))[1:round(m/2)])
}
Vm=sort(Vm)
Vm=Vm[(0.2*sim):(0.8*sim)]


ppp=1-1/sqrt(log(lpm))
Vm=as.numeric(unlist(Vm))
cbd=as.numeric(quantile(Vm,max(ppp,0)))
NPE=ceiling(max(((1:lpm)/lpm-cbd*sqrt(qts1*(1-qts1))-qts1)[1:round(lpm)])*min(n-1,p))
...
BICc[1]=var(y)
for(lp in 1:lpm){
  temp=lm(y~X[,abs(act1[1:(lp+1)])])
  BICc[lp+1]=sum((temp$residuals)^2)+lp*log(p)
}
BIC=which.min(BICc)-1
BIC
BICc
...
match("rs16981663_A",colnames(JPT_CHB_data))
match("rs16981663_T",colnames(JPT_CHB_data))
match("rs16981663_C",colnames(JPT_CHB_data))
match("rs16981663_G",colnames(JPT_CHB_data))
match("rs9981984_A",colnames(JPT_CHB_data))
match("rs9981984_T",colnames(JPT_CHB_data))
match("rs9981984_C",colnames(JPT_CHB_data))
match("rs9981984_G",colnames(JPT_CHB_data))
match("rs7282280_A",colnames(JPT_CHB_data))
match("rs7282280_T",colnames(JPT_CHB_data))
match("rs7282280_C",colnames(JPT_CHB_data))
match("rs7282280_G",colnames(JPT_CHB_data))
match("rs2245431_A",colnames(JPT_CHB_data))
match("rs2245431_T",colnames(JPT_CHB_data))
match("rs2245431_C",colnames(JPT_CHB_data))
match("rs2245431_G",colnames(JPT_CHB_data))
match("rs2832159_A",colnames(JPT_CHB_data))
match("rs2832159_T",colnames(JPT_CHB_data))
match("rs2832159_C",colnames(JPT_CHB_data))
match("rs2832159_G",colnames(JPT_CHB_data))
match("rs1999321_A",colnames(JPT_CHB_data))
match("rs1999321_T",colnames(JPT_CHB_data))
match("rs1999321_C",colnames(JPT_CHB_data))
match("rs1999321_G",colnames(JPT_CHB_data))
match("rs2832224_A",colnames(JPT_CHB_data))
match("rs2832224_T",colnames(JPT_CHB_data))
match("rs2832224_C",colnames(JPT_CHB_data))
match("rs2832224_G",colnames(JPT_CHB_data))
